\documentclass[handout]{beamer}
\usetheme{frankfurt}
\usepackage{tikz}
\usepackage[utf8]{inputenc}
\usepackage{subcaption}
\usepackage{colortbl}
\usepackage{listings}
\usepackage[french]{babel}
\graphicspath{{./}{images/}}
\usetheme{default}
\title{Programmation CUDA}
\author{S. Puechmorel, L. Lapasset}
\date{2023}
\newcommand{\nvglobal}{\texttt{\_\_global\_\_}}
\newcommand{\nvdevice}{\texttt{\_\_device\_\_}}
\newcommand{\nvhost}{\texttt{\_\_host\_\_}}
\newcommand{\nvshared}{\texttt{\_\_shared\_\_}}
\pgfdeclareimage[interpolate=true, height=0.155\paperwidth,width=\paperwidth]{bloc_enac}{images/Bloc_marque_RF_ENAC_V2.png}
\pgfdeclareimage[interpolate=true, height=40pt]{bloc2_enac}{images/Bloc_marque_RF_ENAC_V1.png}
\setbeamertemplate{background}{
  \begin{tikzpicture}
    \ifnum\thepage=1%
  \useasboundingbox (0,0) rectangle (\the\paperwidth,\the\paperheight); 
    \pgftext[at=\pgfpoint{0}{\paperheight-0.155\paperwidth},left]{\pgfuseimage{bloc_enac}};
    \else
\useasboundingbox (0,0) rectangle (\the\paperwidth,\the\paperheight); 
    \pgftext[at=\pgfpoint{0}{\paperheight-40pt},left]{\pgfuseimage{bloc2_enac}};
    \fi

  \end{tikzpicture}
}

\setbeamertemplate{frametitle}{
 \ifnum\thepage>1%
  \rightline{\insertframetitle}
 \fi   
}



\begin{document}
%1
\begin{frame}
    \titlepage

\end{frame}
%2
\begin{frame}
    \frametitle{Plan}
\tableofcontents
\end{frame}
\input{history.tex}
\input{model.tex}
\input{programming.tex}
\input{stream.tex}
\input{unified.tex}
\end{document}
